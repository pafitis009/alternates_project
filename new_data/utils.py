...
def opt_01(quotas, panel, pool, dropout_samples, alternates, columns):
    ...
    return alt_set, prob.objective_value / len(dropout_samples)

def get_loss_of_specific_alternates(panel, quotas, pool, alternates, dropped):
    prob = Model(sense=MINIMIZE)
...
def get_best_loss(panel, quotas, pool, alternates):
    prob = Model(sense=MINIMIZE)
    x = {i: prob.add_var(name=f"x_{i}", var_type=BINARY) for i in range(pool.shape[0])}
    z = {(feature, value): prob.add_var(name=f"z_{feature}_{value}", var_type='I', lb=0) for (feature, value) in quotas.keys()}
    t = prob.add_var(name="obj", var_type='CONTINUOUS', lb=0)
    # Objective
    prob.objective = t
    dropped = panel[panel['Dropped'] != 'YES']
...
def alternates_real_loss(alternates, quotas, panel, pool):
    print(alternates)
    print(type(alternates))
    print(pool.shape[0])
    alternates = pool.iloc[alternates]
    dropped = panel[panel['Dropped'] != 'YES']

    return get_loss_of_specific_alternates(panel, quotas, pool, alternates, dropped)
